...
(function(){
  ...
  const normalize=v=>clean(v,200).toLowerCase();
  const canonicalTitle=v=>normalize(v).replace(/\b(the|a|an|to|for|of|in|on|at|with|and|or|from|after|before|latest|breaking|official|confirmed|report)\b/g,' ').replace(/[^a-z0-9]+/g,' ').replace(/\s+/g,' ').trim().split(' ').slice(0,12).join(' ');
  ...
  async function loadPersonalization(){
    ...
    try{
      ...
      follows=Array.isArray(f)?f:follows;
      bookmarks=new Set((Array.isArray(b)?b:[]).map(x=>String(x.article_id)));
      saveLocalState();
    }catch(_){}
    renderFollowBar();
  }
  ...
  function forYouMatch(a){
    ...
    return follows.some(f=>hay.includes(normalize(f.entity_value)));
  }

  function applyFilter(){
    const q=search.toLowerCase();
    let rows=groupDuplicates(articles).filter(a=>{
      ...
      if(q&&!articleHay(a).includes(q))return false;
      return true;
    });
  ...
  async function fetchArticles({silent=false}={}){
    ...
    const {data,error}=await sb.from(TABLE).select('id,title,summary,url,image_url,source_name,source_domain,source_verified,canonical_key,category,region,league,club,player,breaking,featured,pinned,published_at,comment_count').eq('published',true).eq('moderation_status','visible').order('pinned',{ascending:false}).order('published_at',{ascending:false}).limit(180);
  ...
  function ingestNewsAlert(a){
    const matching=follows.find(f=>articleHay(a).includes(normalize(f.entity_value)));
  ...
  async function shareArticle(id){
    ...
    try{if(navigator.share)await navigator.share(payload);else{await navigator.clipboard.writeText(url);showToast('News link copied.')}}catch(_){}
  }
  ...
  async function toggleBookmark(id){
    ...
    if(error){if(was)bookmarks.add(key);else bookmarks.delete(key);saveLocalState();applyFilter();showToast(error.message||'Could not save story.','bad');return}
  ...
  async function toggleFollow(type,value){
    ...
    if(!session){showToast('Sign in to follow topics across devices.','bad');setTimeout(()=>location.href='account.html',700);return}
    const sb=await getClient();const {data,error}=await sb.rpc('p2u_news_toggle_follow',{p_entity_type:type,p_entity_value:value});
    if(error){showToast(error.message||'Could not update follow.','bad');return}
    ...
    showToast(data&&data.following?`Following ${value}.`:`Unfollowed ${value}.`);
  }
  ...
  function addTopicFromForm(){
    const type=$('#news-follow-type')?.value||'topic',input=$('#news-follow-input'),value=clean(input&&input.value,100);if(!value){showToast('Enter a club, league, country, player, source or topic.','bad');return}toggleFollow(type,value);if(input)input.value='';
  }
