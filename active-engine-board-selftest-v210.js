...
check('service worker v210',sw.includes("const VERSION='v210';")&&sw.includes("const CACHE_VERSION='predict2u-v210';"));
...
const report={build:'v210',generated_at:new Date().toISOString(),passed:checks.length-failed.length,failed:failed.length,checks};
fs.writeFileSync(path.join(root,'active-engine-board-report-v210.json'),JSON.stringify(report,null,2));
if(failed.length){
