...
const health={
  generatedAt:new Date().toISOString(),version:build,label:engineCount===22?"Operational":"Engine registry check needed",
  status:engineCount===22?"operational":"degraded",dataUpdated:d.DATA_UPDATED||updated,scoresUpdated:d.SCORES_UPDATED||updated,
  engineCount,expectedEngineCount:22,multiEngineCount:4,matchCount:matches.length,liveMatches:live,
...
console.log(`Health generated: ${engineCount}/22 engines, ${matches.length} fixtures, ${live} live.`);
if(engineCount!==22)process.exitCode=1;
